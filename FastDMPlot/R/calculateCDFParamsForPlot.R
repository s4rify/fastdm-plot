calculateCDFParamsForPlot <- function(model.cdf){
  ...
  for (s in seq_along(model.cdf)){
    ...
    RT_corrDARK  [[s]] <- data[[s]]$reactionTime_PT[which(data[[s]]$correct == 1 & data[[s]]$fading_function == DARK)]
    RT_incorrDARK[[s]] <- data[[s]]$reactionTime_PT[which(data[[s]]$correct == 0 & data[[s]]$fading_function == DARK)]
    RT_corrLIGHT[[s]]  <- data[[s]]$reactionTime_PT[which(data[[s]]$correct == 1 & data[[s]]$fading_function %in% LIGHT)]
    RT_incorrLIGHT[[s]]<- data[[s]]$reactionTime_PT[which(data[[s]]$correct == 0 & data[[s]]$fading_function %in% LIGHT)]
