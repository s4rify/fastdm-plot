...
for (i in 1:18){
  ...
  rawdata.light800.rt[[i]] <- subset(data[[i]]$reactionTime_PT, data[[i]]$fading_function == 800)
  rawdata.light0.rt[[i]]   <- subset(data[[i]]$reactionTime_PT, data[[i]]$fading_function == 0)
  rawdata.light800.acc[[i]] <- subset(data[[i]]$correct, data[[i]]$fading_function == 800)
  rawdata.light0.acc[[i]]   <- subset(data[[i]]$correct, data[[i]]$fading_function == 0)
...
for (i in 1:18) {
  ...
  rawdata.light.rt[[i]]  <- subset(data[[i]]$reactionTime_PT, data[[i]]$fading_function==800 | data[[i]]$fading_function==0)
  rawdata.light.acc[[i]] <- subset(data[[i]]$correct,         data[[i]]$fading_function==800 | data[[i]]$fading_function==0)
  ...
  rawdata.NOlight.rt[[i]] <- subset(data[[i]]$reactionTime_PT, data[[i]]$fading_function==-1)
  rawdata.NOlight.acc[[i]] <- subset(data[[i]]$correct, data[[i]]$fading_function==-1)
  # This matrix contains all corresponding accuracy responses to the rt values
